          ...
          listener: (context,LoginState state) {
            if(state is UserLoginSucsessState){
             if(state.userModel.status){
               ...
               CashHelper.saveData(key: 'token', value: state.userModel.userData!.token.toString()).then((value) {
                                    ...
                                    builder: (context) {
                                      return defaultButton(function: () {
                                        if (formKey.currentState!.validate()) {
                                          c.login(userEmail: emailController.text,
                                              password: passwordController.text);
                                          fromLogin=true;
                                        }
                                      }, text: 'Login Now');
                                    }, fallback: (BuildContext context)=> const CircularProgressIndicator(color: defColor ,),
                                  ),
                                  const SizedBox(
                                    height: 5,
                                  ),
